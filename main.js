...
async function fetchConfig() {
  try {
    ...
    if (res.ok) {
      ...
      limiterOn = lbMode;
      // Direct mode: if the deploy shipped a default server URL and the user
      // hasn't set their own yet, adopt it so connecting works out of the box.
      if (allowDirect && json.defaultDirect && !settings.directUrl) {
        settings.directUrl = json.defaultDirect;
