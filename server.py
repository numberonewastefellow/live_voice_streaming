...
# Direct-mode default: pre-fill the Settings "server URL" field with your own
# speech-to-speech realtime endpoint (e.g. ws://5.6.7.8:4567/v1/realtime) so the
# user doesn't have to type it. Only used when NO load balancer is configured
# (LOAD_BALANCER_URL empty). The user can still override it in Settings.
DEFAULT_S2S_URL = os.environ.get("DEFAULT_S2S_URL", "").strip()
...
def config():
    ...
    return {
        ...
        # Pre-filled server URL for direct mode; empty in LB mode (never expose it).
        "defaultDirect": DEFAULT_S2S_URL if not LOAD_BALANCER_URL else "",
        "auth": AUTH_ENABLED,
    }
